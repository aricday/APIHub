...
exports.fetchAuthenticationConfiguration = async (url) => {
...
exports.useAuthenticationConfiguration = (signUpEnabled = DEFAULT_SIGNUP_ENABLED, simpleCredentialsEnabled = DEFAULT_SIMPLE_CREDENTIALS_ENABLED, ssoEnabled = DEFAULT_SSO_ENABLED, termsOfUse = DEFAULT_TERMS_OF_USE) => {
    const { url } = ApiHubContext_1.useApiHub();
    const [configuration, setConfiguration] = react_1.useState({
        signUpEnabled,
    ...
    react_1.useEffect(() => {
        exports.fetchAuthenticationConfiguration(url).then(setConfiguration);
    }, [url]);
    return configuration;
};
//# sourceMappingURL=useAuthenticationConfiguration.js.map